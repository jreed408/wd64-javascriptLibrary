...
What is scope?
    ...
    - scope is how a computer keeps track of all the varianles in a program.
    - scope can also be nested, wjere tjere's an outer scope (global) that encloses an inner scope.
*/
...
//! ORDER MATTERS
// JS reads from top to bottom and acts according to what is being asked. Js is know as a single-threaded language which means it can only exxecute a single task at a time.
...
function reassign() {
    a=30 // we are keeping this variable open and, with the 'let' keyword, we are cpable of changing the value that is currently being help within that variable.
